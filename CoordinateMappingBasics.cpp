...
int CCoordinateMappingBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
    if (m_pBackgroundRGBX)
    {
        if (FAILED(LoadResourceImage(L"Background1", L"Image", cColorWidth, cColorHeight, m_pBackgroundRGBX)))
        {
...
LRESULT CALLBACK CCoordinateMappingBasics::DlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case WM_COMMAND:
			...
			if (IDC_BUTTON_BACKGROUND == LOWORD(wParam) && BN_CLICKED == HIWORD(wParam))
			{
				m_bChangeBackground = true;
			}
			if (IDC_BUTTON_AUTO == LOWORD(wParam) && BN_CLICKED == HIWORD(wParam))
			{
				m_bAutoChange = !m_bAutoChange;
			}
			for (int i = 0; i < n_image; i++){
				if (i + 2000 == LOWORD(wParam) && BN_CLICKED == HIWORD(wParam))
				{
					m_bBG[i] = true;
					break;
				}
                                            ...
                                            const BYTE* pBodyIndexBuffer, int nBodyIndexWidth, int nBodyIndexHeight)
{
        ...
        pBodyIndexBuffer && (nBodyIndexWidth == cDepthWidth) && (nBodyIndexHeight == cDepthHeight))
    {
        ...
        if (SUCCEEDED(hr))
        {
			...
			LPCWSTR bgImages[n_image];
			for (int j = 0; j < n_image; j++)
			{
				string buf = "Background" + to_string(j+1);
				size_t size = buf.length();
				wchar_t * filename = new wchar_t[size + 1];
				MultiByteToWideChar(CP_ACP, 0, buf.c_str(), size, filename, size * sizeof(wchar_t));
				filename[size] = '\0';
				bgImages[j] = filename;
			}
			//Auto-change-image mode
			if (m_bAutoChange)
			{
				t = clock();
				double exe_t = (double)(t - prevt) / CLOCKS_PER_SEC;
				if (exe_t >= t_threshold){
					i++;
					int imagePointer = i % n_image;
					LoadResourceImage(bgImages[imagePointer], L"Image", cColorWidth, cColorHeight, m_pBackgroundRGBX);
					prevt = t;
				}
			}
			//reload background as change background button pressed
			...
			for (int j = 0; j < n_image; j++)
			{
				...
				{
					i = j;
					LoadResourceImage(bgImages[j], L"Image", cColorWidth, cColorHeight, m_pBackgroundRGBX);
					m_bBG[j] = false;
				}
			}
        }
...
HRESULT CCoordinateMappingBasics::GetScreenshotFileName(_Out_writes_z_(nFilePathSize) LPWSTR lpszFilePath, UINT nFilePathSize)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        StringCchPrintfW(lpszFilePath, nFilePathSize, L"%s\\SS-%s.bmp", pszKnownPath, szTimeString);
    }
